...
| M1-03 | 1B | Detecting plausible-but-wrong outputs | pattern | v0.1.0 | drafted | Twyman's law via Kohavi; Shen's taxonomy, where only syntax and schema errors stop a query; control totals from the audit manual; the Twitter and Facebook restatements | 1 session |
...
| M1-06 | 1B | Data quality SLOs and error budgets | pattern | v0.2.0 | drafted | The reliability books say not to pick a target from current performance, and data teams import the mechanics without that rule; process capability as the comparison of the reader's requirement against the pipeline's limits; Airbnb's owner-committed landing times, Uber's tier defaults and Shopify's eight-hour default as the practice it argues with | 1 session |
