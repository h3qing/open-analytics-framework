...
## 2026-08-20 — Modules 3 and 4 split; the run/record line; evaluation settled across three modules

**Decision:** four calls, made from a twenty-two-area research sweep across both modules with every source re-fetched and checked. (a) Module 3 names two halves in this order: **3A, what it can reach** (authentication, permission parity, selective exposure, untrusted text, the session record) and **3B, what it tells you** (grounding, context, evaluation, monitoring, adoption). 3A first, because it is the half that fails irreversibly. (b) Module 3 is ordered by **whether the consequence can be reversed**, not the decision. Module 2 orders by expiry, which does not transfer, since nothing here expires and the agent can be switched off. Granting a credential is perfectly reversible right up until a row leaves under it; a question never logged has no answer three weeks later. 3A opens with a Track 0 of seven units that are the only set mandatory before an agent touches production data. (c) Module 4 names two halves: **4A, what you must be able to show** and **4B, the money numbers themselves**. 4B is not Module 1A applied to money, because these definitions carry a disclosure consequence. (d) The Module 3 / Module 4 line: **Module 3 owns the run, Module 4 owns the record.** When a rule binds the agent, ask what generates it: "the agent must not exceed its caller" is Module 3; "this record must be producible unaltered to an outsider" is Module 4, and Modules 2 or 3 implement it. The short form: Module 3's rules are about the agent, Module 4's bind every identity including the founder's.

**The evaluation settlement,** since five rows across three modules claimed this ground. Module 1 owns building a golden question set (`IG-02`) and spotting a plausible-but-wrong output (`M1-03`). Module 3 runs the suite against the agent as a monitor (`M3-02`), and separately owns the runtime constraint that an answer must reconcile against numbers already trusted, which is a different mechanism with a different cadence. Module 4 uses the same suite as a **gate**: a definition change does not ship until it passes (`M4-01`). One suite, built once, three jobs.

**Alternatives:** leaving Modules 3 and 4 undivided, which stops being defensible at fifty units each; splitting Module 3 on "is the answer right" versus "what may the answer do", which fails the independence test because the rule for how many checks a number needs is keyed to how much you trust the agent; and drawing the 3/4 line at technical versus policy, which fails because Module 4's strongest unit is a grant statement and Module 3's is an adoption question.

**Why:** the same test that produced 1A/1B and 2A/2B produces these, and applying one test four times keeps the framework's structure explainable in a sentence. The ordering rule is the module's own contribution: irreversibility transfers from Module 2 but attaches to the failure rather than the deadline. The author's own positions were tested rather than assumed, and all of them survived with a stated qualifier, which is recorded per unit in the research memo rather than here.
