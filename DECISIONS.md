...
## 2026-08-20 — Module 1A gets a metric taxonomy and its first two patterns (M1-12, M1-13)

**Decision:** Four calls made while building out 1A. (a) **Six categories.** Metrics are grouped as acquisition, conversion, engagement, retention, money, and cross-cutting, stated in the charter as the framework's position rather than as received taxonomy. Every existing topic page and every planned one lands in exactly one category, and the money category is shared with Module 4, which owns the reporting half. (b) **The 1A inventory lists units the topic pages already surfaced,** not new inventions: each candidate pattern in the charter traces to a "Patterns & case studies" entry on a drafted topic page, with its primary sources already verified. (c) **Two patterns drafted as source-synthesis units,** extending the 2026-07-31 exception to hard constraints 6–7: M1-12 (Redefining a metric in the open) and M1-13 (Check attribution by turning it off). Every claim in both cites a public document; both are `drafted`, never `published`, until the author's voice pass. Where each page gives a team-of-ten version, it is marked as the framework's adaptation rather than a reported practice, because no small-company first-party account exists for either. (d) **One pattern, not two, for definition change.** The active-users page had flagged "redefine in the open" and "publish the recast" as separate candidates; they are drafted as a single pattern, because a deliberate redefinition and a discovered break ship the same four things.

**Alternatives:** organize 1A by unit type as the charter previously did, or by the Module 2 track model; write the two active-users candidates as separate pages; wait for interview answers before drafting either pattern; put the money category wholly in Module 4.

**Why:** a reader arrives with a business question ("where did they come from", "do they come back"), which is what the categories are, and grouping by unit type answered a question nobody asks. The Module 2 track model orders units by what expires, which is a property of infrastructure and has no analogue in metric definitions. The two definition-change candidates share a mechanism, a source set and a failure mode, so splitting them would have produced two pages that cite the same three filings and disagree about nothing. And the sources are the answers for a synthesis unit, which is what licensed the same exception in July.
