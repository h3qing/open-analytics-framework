...
## 2026-09-06 — M1-03 argues with the green test suite, reads the output and never the query, and imports the control total from audit

**Decision:** four calls made while drafting the plausible-but-wrong pattern. (a) The Position argues with the passing assertion suite, not with doing nothing. The competent practice it contests is a data-test suite of uniqueness, not-null, accepted-value, and relationship assertions, plus unit tests of model logic on fixed inputs; a team that runs these believes a green run means a right number. The page answers that these test the inputs and the author's own logic, and the plausible wrong number comes from what the author did not anticipate, so the evidence has to come from outside the query. (b) Every check on the page reads the output and never the query. That is what lets one set of checks serve a pipeline's output and an AI agent's answer without a separate agent section. (c) Reconciliation to a control total is imported from accounting and audit practice and said so, with the definition quoted from the federal audit manual; no data-engineering source was found that documents the check as a first-class practice for this audience, and the page does not pretend one exists. (d) The independence rule for the second path, that it must start from material the first query never touched, rests on the 1986 multi-version programming experiment, cited at abstract level and forty years outside the recency preference, because it is the canonical result and nothing newer replaces it. The Twitter recast already told on the active-users page is used here as one-line evidence only, with the check that was missing, and is not retold.

**Alternatives:** argue with the data-observability purchase, which this audience has mostly not made; argue with doing no checks, which no competent practitioner defends in print; write a separate section for agent answers, which would duplicate the checks with the word agent inserted; cite a vendor's reconciliation feature as the lineage of the control total, which would name a product and misplace the origin.

**Why:** a Position should contest the best practice the reader actually follows, and the green suite is what a small team with a modern transformation tool has. Reading the output keeps the page to one method. Naming the audit lineage is both honest and useful, since a founder's accountant already knows the check by that name.
