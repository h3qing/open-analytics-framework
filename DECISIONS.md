...
## 2026-09-06 — M1-04 argues with two practices at once, states a floor of twenty, and makes refusal a required member

**Decision:** three calls made while drafting the golden-question-set pattern. (a) The Position argues with two competent practices rather than one, because the reader hears both: validating an AI answer by reading its query, whose strongest published form is a business-intelligence vendor's transparency argument, and choosing a tool by its public benchmark score. Each is taken at its strongest. Showing the query survives as the audit artifact, in line with the Module 4 position that the executable query is the thing to send; only reading it as the test is rejected, on the evidence that experts split on what counts as an acceptable query and that model checkers lean toward calling wrong queries equivalent. The benchmark score is granted as evidence about models and denied as evidence about the reader's tables, on the two annotation audits and a vendor's own drop from a leaderboard figure to its internal set. (b) Twenty questions is stated as the framework's floor, since no published account gives one; the published sizes run from about thirty hand-graded labels to five hundred stored questions, and the floor sits where one person can verify every value by hand. (c) The composition rule is the framework's own: the numbers that leave the room, the known traps, at least one refusal, and every wrong answer already seen in production. Refusal is required because no published suite tests that the agent declines, and for a company with no analyst the confident answer about uncollected data is the failure nobody else catches. The rule that a failure is a finding about the context, then the model, then the data follows the paired benchmark in which a context document moved three frontier models by the same margin while the models were indistinguishable.

**Alternatives:** argue only with reading the query, which leaves the benchmark shopper unanswered, or only with the benchmark, which this audience follows less often; leave the size to the reader, which the research memo shows produces sets of a hundred that nobody verifies; make refusal optional, which every published suite already does.

**Why:** a Position should meet the best version of what the reader is actually doing, and this reader does both. The floor and the composition rule are the two places the published accounts are silent and the reader most needs an answer, so the framework states its own and marks them as its own in the body and in Sources & Stories.
