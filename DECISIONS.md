...
## 2026-09-11 — M1-07 moves the contract check to the landing point, and carries a transfer it cannot fully close

**Decision:** three calls made while drafting the schema-drift pattern. (a) The Position argues with the producer-side data-contract programme, using the programme's own practitioners against it rather than an outside critic. The rule it contests is stated plainly in the Convoy implementation guide, that a contract must be enforced at the producer level and anything short of that is a handshake agreement, and in the GoCardless post that catalogs and anomaly detection do nothing to improve data quality. The counter-evidence is also theirs: the GoCardless six-month report records that contracts reached about half of inter-service communication and did not reach data produced for the data teams, and the same Convoy practitioner later wrote that starting in the warehouse needs no application engineers and a smaller culture shift. The second half of the position, contracting only the fields a metric reads, is the framework's own and depends on M1-01 having already produced that field list. (b) The page's central asymmetry, structural changes announcing themselves and semantic changes not, rests on Google's anomaly frequencies from machine-learning pipelines, since no equivalent published measurement was found for warehouse metric pipelines. The transfer is argued in the body and flagged in Sources & Stories rather than assumed. This is the page's weakest joint and the first thing to revisit if a metric-pipeline source appears. (c) Backfills are contracted as closed-period stability, and the template therefore carries a frozen-snapshot table, because a check that only inspects today's load cannot see history being rewritten.

**Alternatives:** argue with doing nothing, which no competent practitioner defends in print, so the Position would contest a strawman; argue with the schema-registry purchase, which this audience has mostly not made, so it survives as a failure mode instead. For the asymmetry, wait for a warehouse-native measurement, which would leave the unit unwritable. For backfills, hand them to the reconciliation unit (M1-08), which would leave the most common quiet number-mover off the page that exists to catch quiet number-movers. Tracking plans were also cut here rather than covered, since M2-08 owns that template.

**Why:** a position should contest the best advice the reader will actually hear, and for this unit that advice comes from the engineers who ran the programmes and published honestly about what it cost them. Naming the transfer instead of hiding it keeps the page usable by someone who wants to check it.
