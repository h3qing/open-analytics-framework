...
## 2026-08-15 — Retention is organized around the two-kinds split, not the concept

**Decision:** Four calls made while drafting the retention topic page. (a) The page's spine is the author's entity-versus-value split rather than the conventional definition-then-visualization order, and the LTV section moves below both halves because the sorting effect and the NRR construction mess are both inputs to it. (b) The published contradiction over smiling retention curves is resolved in the framework's own voice, by noting that a survival curve, an activity curve and a dollar curve are three different quantities and that a smile means something different on each. (c) Sequoia's retention article gets the suffixed key `SEQUOIA-2018B`, because the existing `SEQUOIA-2018` entry already resolves to a different verified article by the same team in the same month. (d) Five new reference entries carry `[TODO: verify]` for missing titles or links, following the REFERENCES convention rather than reconstructing plausible bibliography.

**Alternatives:** keep the stub's section order and treat the split as one section among several; report the smile disagreement without resolving it; overwrite the existing Sequoia entry; infer the missing titles from URL slugs and well-known citations.

**Why:** the split is what makes the rest of the page decidable, since a reader who knows which kind they are measuring knows which half to read and which arguments do not apply to them. The smile contradiction is the clearest place where this framework can add something the sources do not contain, and leaving it unresolved would have been a literature review. And a verified entry that points at the wrong article is worse than a suffixed key, while a reconstructed title is exactly the plausible-looking reference the conventions forbid.
