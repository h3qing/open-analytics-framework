...
Every attribution model is one assumption about how credit should flow, written down as arithmetic. No model measures what caused what. The models are samples of possible opinions, not truths, and the cleanest way to see that is to run them all on the same journey.
...
| Marketing mix modeling | Does not look at this journey at all. It fits a statistical model relating weekly outcome totals to weekly channel spend, with no user-level tracking. | | | |
| Incrementality | Answers a different question: what would have happened without a given touch. That takes a holdout, a group deliberately not exposed, not a credit rule. | | | |

Five rules, five different answers, one journey, and nothing was measured. The gap between a credit rule and a measurement can be enormous: eBay's attribution-era belief was that paid search drove about 5 percent of its sales; when the company actually held the ads out and looked, the answer was about 0.5 percent, not statistically different from zero [[FREAKONOMICS-2020]](../../REFERENCES.md) [[BLAKE-2015]](../../REFERENCES.md).
...
**Marketing mix modeling.** MMM for short: regression (statistical line-fitting) of outcome totals against channel spend over time, adjusting for things like seasonality. No individual user is tracked, so no journey is credited. It answers "what does a dollar in this channel appear to buy," slowly and with error bars.

**Incrementality.** The experimental answer. Hold out a region or a group, keep spending everywhere else, and compare. eBay's version was geographic: paid search off in a third of US media markets, sales compared against the rest [[FREAKONOMICS-2020]](../../REFERENCES.md).

Kaushik's 2013 advice to big spenders already drew the graduation path this page endorses: past roughly $10 million of marketing spend, skip the attribution-model debates and go to media-mix modeling and controlled experiments [[KAUSHIK-2013]](../../REFERENCES.md).

## Classic marketing attribution

**The last-click decade.** For roughly a decade the default answer to "which channel gets credit" was the last click, because it was the click the tools could see. The distortion is structural: last click rewards the channel that harvests intent and starves the channels that create it. Seufert argued before the privacy shocks that attribution was already withering on its own, because the channels marketers were diversifying into, TV, influencers, out-of-home, cannot be measured at the level of the individual user at all [[SEUFERT-2017]](../../REFERENCES.md). His later diagnosis of why teams clung to it anyway: last-click attribution gave advertisers a veneer of control "like a security blanket," a measurability that was really an illusion [[SEUFERT-2023]](../../REFERENCES.md).

**Then the signal went away.** The chronology runs from the first limits on mobile ad identifiers in 2012, through identifier zeroing in 2016, GDPR in 2018, Apple's app-tracking consent pop-up enforced in 2021, and the announced deprecation of third-party cookies [[SEUFERT-2023]](../../REFERENCES.md). In 2023 Google retired first-click, linear, time-decay, and position-based attribution from its ads products, saying fewer than 3 percent of conversions still used them, which left last-click and its own data-driven model; advertisers publicly worried that the black box might favor the vendor's own channels [[OSMUNDSON-2023]](../../REFERENCES.md). The vendor that popularized the rule-based models retired most of them.

**The MMM renaissance.** With user-level signal dying, marketing mix modeling came back into fashion, and its pedigree is worth knowing: "marketing mix" was coined in the early 1950s, operations researchers were modeling promotional effort by 1953, and the first regression-based advertising-profitability work dates to 1969, all decades before the click [[PEDANTICS-2023]](../../REFERENCES.md). The renaissance has a wrinkle: the largest ad platforms now publish their own free, open-source MMM frameworks, and independent trade reporting frames the question to ask about that, whether measurement can be trusted when it is provided by the same players who profit from the results [[HERCHER-2026]](../../REFERENCES.md). Seufert adds the operational honesty: econometric output does not plug into a last-click-shaped dashboard, it comes with uncertainty and needs interpretation [[SEUFERT-2023]](../../REFERENCES.md).
...
**Agree on one number first.** Lemkin's advice for small companies is upstream of any credit rule: the VP of sales and VP of marketing jointly pick one pre-closed-won number as marketing's top goal, define the qualified-lead threshold together, and keep their disagreements private [[LEMKIN-2021]](../../REFERENCES.md). Before you design a credit split, agree on a shared number.

**The early-stage playbook.** Emily Kramer's setup for early B2B teams: first-touch as the primary lens, last-touch at the qualified stage as a supplement, inbound and outbound broken apart, all built on UTM tags (the tracking labels appended to links) and CRM hygiene rather than attribution software. Her warning matches this page's: you do not need a complex multi-touch model to figure out what is actually going on [[KRAMER-2023]](../../REFERENCES.md).

**The position this framework takes: attribution is incentive design.** In a go-to-market organization, attribution is the measurement layer that compensation and OKRs (objectives and key results, the goal-scoring system) sit on. How an SDR, a marketing team, or a partnerships team drives revenue is how they get compensated and how their OKRs get measured. The credit rule is the input to quota attainment and performance review, which is to say the attribution model is really your compensation model. A wrong rule therefore does more than misallocate budget: it mis-pays people and mis-scores teams, and behavior follows the rule either way. Accurately measuring the OKR is important in its own right, not only for the budget's sake. To be explicit about sourcing: no published literature states this claim in this form. The searches were run, and what exists nearby is vendor content about sales compensation. The claim is this framework's own position. The canon backs the mechanism from three directions.

**Rewarding A while hoping for B.** Steven Kerr's 1975 management classic describes reward systems that pay for one behavior while the organization hopes for another [[KERR-1975]](../../REFERENCES.md). A sourced-pipeline target that pays marketing for pipeline created, while the company wants revenue closed, is exactly Kerr's shape, and Kellogg's pipegen example is what it looks like on a dashboard.

**The measure that is born a target.** The famous line, "When a measure becomes a target, it ceases to be a good measure," is usually attributed to the economist Charles Goodhart. The provenance is worth telling straight. The sentence is Marilyn Strathern's, verbatim, from a 1997 paper on audits in British universities, and she in turn credits the naming of "Goodhart's law" to Keith Hoskin [[STRATHERN-1997]](../../REFERENCES.md). What Goodhart wrote in 1975 was drier: "Any observed statistical regularity will tend to collapse once pressure is placed upon it for control purposes." [[GOODHART-1975]](../../REFERENCES.md) Attribution is the special case where the measure does not become a target. It is born as one, because the entire reason to compute a credit split is to score people and spend. That is why Kellogg's ledger fix matters: if the number pays people, the ledger cannot be kept by the teams being scored.
...
**The under-$50M argument.** Fishkin's blunt version: for the large majority of businesses, the ones under $50 million in annual revenue, he does not believe investments in attribution software and multi-touch modeling return a profit [[FISHKIN-2022]](../../REFERENCES.md). His alternative is not "no measurement." It is a working list of channels proven to drive results, secondary metrics and traffic lift around experiments, and a gut made trustworthy by real customer knowledge, backed by enough baseline analytics to catch disasters [[FISHKIN-2022]](../../REFERENCES.md).
...
The two ends of the path agree, and that is the point. Below some scale, attribution machinery costs more than the decisions it informs are worth. Above some scale, experiments answer the question better. The band where rule-based attribution modeling earns its keep is much narrower than the tooling industry suggests. Last-touch plus honest channel tests carries a small team a long way; the [conversion page](conversion.md) covers where last-touch enters.
...
- **The pipegen mirage.** Six quarters of marketing hitting 105 percent of its pipeline target while sales slid to 82 percent of plan: hit the proxy, miss the point [[KELLOGG-2024]](../../REFERENCES.md).
...
One honest research finding belongs on the record: no verified first-party story exists of a small team, this framework's audience, changing its attribution approach and reporting what happened. Every verified story above is a giant with an experimentation budget. The literature over-samples companies big enough to run holdouts, and this page says so rather than pretending otherwise.
