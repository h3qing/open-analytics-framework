...
- **Plain sentences, one clause each.** No noun-phrase fragments with a qualifier hung off the end, no “, which is…” trailed after a finished thought, no clause doing two jobs. Write “There are five steps, and no model is needed” rather than “Five steps, none of which need a model.” The full rule is in `AGENTS.md`.
