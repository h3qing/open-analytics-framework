...
A column gets dropped or renamed and the dashboard is blank on Monday. That one is cheap, because somebody finds it the same morning from the failure. A status field grows a fifth value, a price starts arriving in dollars instead of cents, or a field stops being populated for one source, and nothing fails. The consumer does not crash. It keeps running and gets worse, and the number keeps coming, a little wrong, until a customer or the board asks.[^polyzotis-2018]

A ten-person company gets that failure without the machinery for catching it. The person who renames the column is often the person who owns the metric, and at that moment they are thinking about the application rather than Monday's number.

There is a newer version of the same failure, and I think it is the one to plan for now. An AI assistant does not rescan the warehouse on every question. It keeps notes: which table holds revenue, which four values the status field takes, that amounts are in cents. Those notes were written from the data as it was on the day the assistant looked. When the field changes, the data has drifted and the assistant's memory has not, and from then on the memory is what produces the wrong answer, on every question that touches that field, with no sign of doubt. This drift is not on the traditional list, and no test on the pipeline catches it, because the pipeline is fine.
...
- The assistant still answers with the four status values it learned in March, and the field has had five since June.
...
The [control plan template](../../../templates/01-ai-data-quality/schema-contract-control-plan.md) carries the field inventory, the check table, a machine-readable core, and the agent prompts. Steps 1 to 3 cost an afternoon and then a few minutes a week. Steps 6 and 7 are the ones I would not skip.
...
2. **Write down what must stay true about each one, as a check that can fail.** The structural line is short and your tooling already carries its tests. The semantic line is the one I would spend the afternoon on. Write the unit and the timezone in words as well, because a field that changes from days to hours is undetectable unless somebody recorded which it was.[^polyzotis-2018] The published vocabulary calls these completeness and consistency.[^schelter-2018]

   | What must stay true | The check | Where it usually lives |
   |---|---|---|
   | The field is present and populated | `not_null` on the field, and a row count above zero for the load | the generic tests in transformation tooling[^dbt-data-tests] |
   | The key identifies one row | `unique` on the key | the generic tests[^dbt-data-tests] |
   | The type is what the query expects | a declared column type, enforced when the model builds | model contracts[^dbt-contracts] |
   | A categorical field holds only known values | `accepted_values` with the full list your `CASE` handles | the generic tests[^dbt-data-tests] |
   | A foreign key points at a real row | `relationships` to the parent table | the generic tests[^dbt-data-tests] |
   | A number stays inside its bounds | one assertion in SQL: no negative amounts, no order above the largest ever seen | a singular test[^dbt-data-tests] |
   | The unit and the timezone are what the contract says | the words in the contract, plus one bound that would fail if cents became dollars | the contract file and one assertion |
   | Null share, row count and freshness stay in their usual range | a control chart, not a threshold | [statistical process control](statistical-process-control-for-pipelines.md) |
   | A closed period returns the same numbers | the hand-checked questions on closed periods, re-run offline | [golden question sets](golden-question-sets.md) |
...
6. **Re-run the offline tests before the change ships, and again after.** The twenty hand-checked answers on closed periods must return the same values after a schema or definition change. That is what keeps history reproducible. The check that a new model re-derives a blessed metric to the same number is what keeps an existing column's definition undisturbed by a change meant for a new one. Both already exist on the [golden question sets](golden-question-sets.md) and [single point of metric computation](single-point-of-metric-computation.md) pages. Here they are the gate, and a semantic change that slipped past every field-level check still shows up as a known-correct answer that stopped being correct. The page on [catching a plausible number that already left the room](plausible-but-wrong.md) covers the case where it slipped past this too.
7. **Update what the assistant remembers, in the same commit.** When a contracted field changes, the [context file](../03-ai-agent-integration/context-file.md) line for that field changes with it: the allowed values, the unit, the table to use. If the assistant keeps notes of its own, delete the ones that mention the field and let it look again. A contract check that fires and leaves the memory alone has fixed the pipeline and left the wrong answer in place.
...
- **The assistant remembers the old contract.** The field changed and the assistant's notes did not, so every answer that touches the field is wrong and reads as sure. This one is my own addition to the usual list. The memory is part of the contract, and step 7 is the fix.
...
The Google thread is three papers from one group: keeping an inferred schema beside the pipeline's code, from the peer-reviewed account of the data-validation system inside its machine-learning platform [^breck-2019]; the unit change, the South Sudan constraint, and the finding that a consumer degrades rather than crashing, from the SIGMOD Record survey [^polyzotis-2018]; and the split between a pipeline's schema and the process generating the data, from the SIGMOD tutorial [^polyzotis-2017]. All three describe machine-learning pipelines; the transfer to a warehouse metric is mine. The constraint vocabulary is Amazon's [^schelter-2018], and the generic test names are from the transformation tool's own documentation [^dbt-data-tests].
...
Contracting a metric's own fields, the weight on the semantic half, the reaction plan at ten people, and the assistant's memory as part of the contract are mine. No published first-party account applies data contracts at that size. The prior-art row is pending in [prior-art.md](../../prior-art.md).
