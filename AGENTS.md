...
- **Write the content, never the making of the content.** Reader-facing pages say what a thing is and what the reader gets from it. Module boundary settlements, cross-module comparisons, authoring-sequence notes, and lists of what a page does not cover belong in `DECISIONS.md` and the backlog, never in charters or indexes. A scope line says what the module covers, and stops.
...
- **One pasted line gets a reader in.** `llms.txt` at the root is what an AI assistant reads after the reader pastes the line on the README. Every finished page and template gets a row there, written as the problems the page is for, in the reader's words, and the row is rewritten whenever the page's problems or position change. CI fails when a row is missing; a stale row is on the person editing the page.
...
Branch names are plain and descriptive (`entity-retention`), never tool-prefixed. Commits and pull requests carry no co-author trailers and no generated-by footers. Messages lead with the problem the change solves, in ordinary prose, so the history reads as one maintainer's work. Pull request descriptions say what the change is and what it adds; the reasoning behind it lives in `DECISIONS.md`.
