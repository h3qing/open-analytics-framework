...
- **Plain sentences, one clause each.** Every sentence gets a subject and a verb. Do not build a sentence as a noun phrase with a qualifier hung off the end (“Five steps, none of which need a model”); write the sentence out (“There are five steps to calculating lifetime value, and no model is needed”). Do not trail “, which is…” after a finished thought; start a new sentence. Fragments do not stand in for sentences, and abbreviations stay expanded in running prose outside their definitions and tables. A strong instruction carries its reason in the same breath (“Do not build a valuation model, because everything it would tell you is already in the cohort table”), never as a bare command. Do not narrate what the format already shows: a numbered list needs no “in this order”. Cut clauses that add no information (“and all of them are free”). The appositive-fragment register and the empty-clause padding both read as generated text; the test is whether a paragraph reads like a knowledge article a person wrote down.
